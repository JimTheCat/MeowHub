...
public class ChatMessageService {
    ...
    public ChatMessageDto save(ChatMessageDto chatMessageDto) {
        ...
        Chatroom chatroom = chatRoomService.getChatroom(chatMessageDto.getChatroomId())
                .orElseThrow(() -> new RuntimeException("Could not find chatroom with id: " + chatMessageDto.getChatroomId()));
    ...
    public List<ChatMessageDto> findChatMessages(String chatroomId, int page, int size) {
        ...
        return repository.findByChatroomIdOrderByCreatedAtDesc(chatroomId, pageable).stream().map(chatMessage -> {
            ...
            String messageReceiver = messageAuthor == chatroom.getReceiver().getLogin() ? chatroom.getSender().getLogin() : chatroom.getReceiver().getLogin() ;
